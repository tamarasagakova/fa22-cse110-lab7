...
function initializeServiceWorker() {
  ...
  if ('serviceWorker' in navigator) {
    ...
    window.addEventListener('load', () => {
      ...
      console.log(`Service Worker has been successfully registered: ${registration}`);
      },
...
async function getRecipes() {
  ...
  RECIPE_URLS.forEach(async (url) => {
    ...
    try {
      ...
      //            "await" again
      let json = await fetch_url.json();
      // A8. TODO - Add the new recipe to the recipes array
      recipes_to_fetch.push(json);
      ...
      //            resolve() method.
      if (recipes_to_fetch.length == RECIPE_URLS.length) {
        saveRecipesToStorage(recipes_to_fetch);
        resolve(recipes_to_fetch);
      }
    }
